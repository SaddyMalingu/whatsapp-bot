...
app.post("/webhook", loadTenantContext, async (req, res) => {
  ...
  try {
    ...
    // STEP 3b: Prefetch DB context for LLM
      ...
      const [userProfile, brandProfile, catalogMatches, userPurchases, userMessages] = await Promise.all([
        fetchUserProfile(userData.id),
        fetchBrandProfile(brandId),
        tenantPhone ? fetchCatalogForTenant(tenantPhone, text) : Promise.resolve([]),
        fetchUserPurchases(userData.id, 5),
        fetchUserMessages(userData.id, 5),
      ]);
...
async function generateReply(userMessage, tenant = null, templates = null, trainingData = [], contextMessages = [], catalogMatches = [], dbContext = "") {
  ...
  // --- Configurable LLM provider fallback order ---
  // Example: process.env.LLM_PROVIDER_ORDER = "hf,openai,openrouter"
  const providerOrder = (process.env.LLM_PROVIDER_ORDER || "hf").split(",").map(p => p.trim().toLowerCase());
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          if (attempt > 0) {
            ...
            log(`Retrying HuggingFace (attempt ${attempt + 1}) after ${delay}ms`, "WARN");
          ...
          if (hfResponse.data?.choices?.length > 0) {
            ...
            lastErr = new Error("No choices returned");
          }
        } catch (hfErr) {
          lastErr = hfErr;
          log(
            `HuggingFace error: ${hfErr.message} (attempt ${attempt + 1})\nProvider: huggingface\nInput: ${JSON.stringify({ model: "meta-llama/Llama-3.1-8B-Instruct:novita", messages: messageStack })}\nStack: ${hfErr.stack}`,
            "ERROR"
          );
          // Only retry on network or 5xx errors
          if (!hfErr.isAxiosError || (hfErr.response && hfErr.response.status < 500)) {
            break;
          }
        }
      }
    }
    // Future: add openai, openrouter, etc. here as needed
  }

  // 4️⃣ Static fallback message
  log("All LLM providers failed: none succeeded in providerOrder", "ERROR");
  return {
    type: "text",
...
// Fetch last N purchases for a user
async function fetchUserPurchases(userId, limit = 5) {
  if (!userId) return [];
  const { data, error } = await supabase
    .from("purchases")
    .select("id, product_sku, product_name, amount, currency, created_at")
    .eq("user_id", userId)
    .order("created_at", { ascending: false })
    .limit(limit);
  if (error) {
    log(`User purchases error: ${error.message}`, "WARN");
    return [];
  }
  return data || [];
}
...
async function fetchUserMessages(userId, limit = 5) {
  if (!userId) return [];
  const { data, error } = await supabase
    .from("conversations")
    ...
    .limit(limit);
  if (error) {
    log(`User messages error: ${error.message}`, "WARN");
    return [];
  }
  return data || [];
}
