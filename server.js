...
import { startHealthMonitor, incrementErrorCount, runHealthCheck } from "./utils/healthMonitor.js"; // ✅ combine here
...
// ✅ Start health checks
startHealthMonitor(ADMIN_NUMBERS, 200);  // routine 6-hour checks
runHealthCheck(ADMIN_NUMBERS);           // one-time immediate check


let pendingClearConfirmations = {}; // stores pending confirmations
...
app.post("/webhook", async (req, res) => {
  ...
  if (body.object) {
    ...
    const text = message?.text?.body?.trim();
      ...
      // Check if awaiting confirmation (one-time 'YES' for clearlogs)
      if (pendingClearConfirmations[from] && text.trim().toUpperCase() === "YES") {
      ...
      if (text.startsWith("/diagnose ")) {
        ...
        // Normal AI conversation (fallback)
...
async function handleClearLogs(from, text) {
  ...
  await sendMessage(
    from,
    "⚠️ Are you sure you want to clear logs? Reply with 'YES' within 30 seconds to confirm."
  );
...
async function handleHealthCheck(from, text) {
  ...
  try {
    ...
    // Run the regular check so the logs file still updates
